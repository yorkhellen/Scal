...
int main (int argc, char ** argv)
{
    ...
    current_size = user_opts->buf_size;

    count = 1024 * 1024 * 1024 / user_opts->buf_size;
    time1 = Wtime();
    while(count > 0)
    {
...
static struct options* parse_args(int argc, char* argv[])
{
	...
	switch(one_opt){
            case('v'):
                printf("%d.%d.%d\n", PVFS2_VERSION_MAJOR, PVFS2_VERSION_MINOR, PVFS2_VERSION_SUB);
                exit(0);
	    case('t'):
		tmp_opts->show_timings = 1;
		break;
...
static void print_timings( double time, int64_t total)
{
    printf("Wrote %lld bytes in %f seconds. %f MB/seconds\n",
	    (long long int)total, time, (total/time)/(1024*1024));
}
